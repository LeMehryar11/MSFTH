...
- **`index.html`** — the parameter-led interactive ROI dashboard for the client.
- **`calculator.html`** — a compatibility link that opens the dashboard.
- **`pitch-deck.html`** — the executive pitch deck.

See `map.md` for the full file listing and `usage.md` for how to run the dashboard and deck.
...
Open the local address printed in the terminal. The dashboard lists the model's fixed facts and placeholders first, then exposes employees, time horizon, migration effort and time saved as live controls.
...
This runs Node's built-in test runner against `roi-model.js`.
